...
analysis_outdir = "Analysis_navierstokes_shockcapturing_$(run_label)"
solution_outdir = "out_navierstokes_shockcapturing_$(run_label)"
...
# power injected by the forcing, rho*(v.a)
...
mutable struct DissipationAccumulator
    total::Float64
    at_last_report::Float64
    t_last_report::Float64
    cool::Float64
...
###############################################################################
# solver with shock capturing (Hennemann-Gassner indicator)
...
###############################################################################
# Isothermal temperature reset callback
...
try
    global sol = solve(ode, CarpenterKennedy2N54(williamson_condition = false);
